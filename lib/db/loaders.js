...
const deprecatedCourseSlugs = new Set(["working-with-data-for-professionals", "ai-mastery"]);
const localCourseBySlug = Object.fromEntries(ubuntuCourses.map((course) => [course.slug, course]));
...
function mapProgram(row, index = 0, mentors = mentorProfiles) {
  ...
  const localCourse = localCourseBySlug[row.slug] || {};
  ...
  return {
    id: row.id,
    slug: row.slug,
    title: override.title || localCourse.title || row.title,
    category: localCourse.category || row.category,
    courseType: localCourse.courseType || row.course_type || metadata.course_type || "course",
    level: localCourse.level || row.level,
    summary: localCourse.summary || row.summary,
    heroSubtitle: localCourse.heroSubtitle || metadata.hero_subtitle || metadata.heroSubtitle || "",
    priceKes: override.priceKes || localCourse.priceKes || Number(row.price_kes || 0),
    oldPriceKes: override.oldPriceKes || localCourse.oldPriceKes || null,
    duration: localCourse.duration || row.duration,
    startDate: override.startDate || localCourse.startDate || row.start_date || null,
    displayOrder: Number(localCourse.displayOrder || row.display_order || index + 1),
    isActive: row.is_active !== false,
    mentor,
    instructor: localCourse.instructor || metadata.instructor || mentor.name,
    instructorTitle: localCourse.instructorTitle || metadata.instructor_title || mentor.title,
    modules: safeArray(localCourse.modules, safeArray(metadata.modules)),
    outcomes: safeArray(localCourse.outcomes, safeArray(metadata.outcomes)),
    journeySchedule: safeArray(localCourse.journeySchedule, safeArray(metadata.journeySchedule)),
    tools: safeArray(localCourse.tools, safeArray(metadata.tools)),
    projects: safeArray(localCourse.projects, safeArray(metadata.projects)),
    roles: safeArray(localCourse.roles, safeArray(metadata.roles)),
    demand: safeArray(localCourse.demand, safeArray(metadata.demand)),
    schedule: override.schedule || localCourse.schedule || metadata.schedule || ""
  };
}
...
function mapCatalogProgram(course, index = 0) {
  ...
  return {
    ...
    oldPriceKes: override.oldPriceKes || course.oldPriceKes || null,
    ...
    journeySchedule: safeArray(course.journeySchedule),
    tools: course.tools,
...
function mergeLocalPrograms(remotePrograms) {
  const activeRemotePrograms = remotePrograms.filter((program) => !deprecatedCourseSlugs.has(program.slug));
  const seen = new Set(activeRemotePrograms.map((program) => program.slug));
  ...
  return [...activeRemotePrograms, ...missing].sort((a, b) => a.displayOrder - b.displayOrder);
}
